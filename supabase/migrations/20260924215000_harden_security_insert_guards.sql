-- Harden the insert guards while preserving database-generated id/created_at defaults.
-- Direct clients still cannot invent arbitrary approvals: the canonical action must
-- be inside a live mandate and actually require approval.
create or replace function public.guard_approval_insert()
returns trigger language plpgsql security invoker set search_path=''
as $$
declare m public.mandates; v_agent_id uuid; v_action text; v_amount numeric; v_currency text; canonical jsonb; expected_fp text;
begin
 ...
 begin v_agent_id:=nullif(new.action_payload->>'agentId','')::uuid; exception when others then raise exception 'invalid_action_payload'; end;
 ...
 if new.action_payload ? 'amount' and new.action_payload->'amount'<>'null'::jsonb then
  ...
  v_amount:=(new.action_payload->>'amount')::numeric; if v_amount<0 then raise exception 'invalid_action_payload'; end if;
 end if;
 if new.action_payload ? 'currency' and new.action_payload->'currency'<>'null'::jsonb then
  ...
  v_currency:=upper(btrim(new.action_payload->>'currency')); if v_currency !~ '^[A-Z]{3}$' then raise exception 'invalid_action_payload'; end if;
 end if;
 select x.* into m from public.mandates x join public.organizations o on o.id=x.organization_id
 where x.id=new.mandate_id and x.organization_id=new.organization_id and o.owner_id=(select auth.uid()) and x.revoked_at is null and x.expires_at>now();
...
begin
 ...
 select x.* into a from public.approvals x join public.organizations o on o.id=x.organization_id
 where x.id=new.approval_id and o.owner_id=(select auth.uid()) and x.decision='APPROVED' and x.used_at is not null and x.expires_at>now();
 if a.id is null then raise exception 'approval_unavailable'; end if;
 if new.organization_id<>a.organization_id or new.mandate_id<>a.mandate_id or new.action_fingerprint is distinct from a.action_fingerprint or new.action_payload is distinct from a.action_payload
